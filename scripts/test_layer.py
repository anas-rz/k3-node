...
# Usage: KERAS_BACKEND=<backend> python scripts/test_layer.py <LayerName>

# Default to CPU to prevent GPU OOM on machines with limited VRAM
...
def get_layer_test(layer_name):
    ...
    std_convs = {
        ...
        "EGConv": lambda: k3_layers.EGConv(in_c, out_c),
    }
    ...
    if layer_name == "HypergraphConv":
        def model_factory():
            ...
            return Model()
        def inputs_factory():
            # hyperedge_index: (2, num_nodes_in_edges)
            ...
            class Model(keras.Model):
                ...
                def call(self, inputs):
                    ...
                    if layer_name == "EdgePooling":
                        ...
                        # DMoNPooling takes dense batch (batch, N, in_c) and adj (batch, N, N)
            ...
            class Model(keras.Model):
                ...
                def call(self, inputs):
                    ...
                    if layer_name in ["PerformerAttention", "PolynormerAttention"]:
                        ...
                        # x: (1, N, in_c) -> expand to (1, 1, N, in_c)
...
def run_test(layer_name):
    ...
    try:
        ...
        return {
            ...
            "initial_loss": l0,
            "final_loss": l_last,
            "loss_reduced": learned,
        }
    except Exception as e:
        tb = traceback.format_exc()
        return {"status": "FAIL", "error": str(e), "traceback": tb}
